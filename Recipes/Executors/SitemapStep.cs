    ...
    public class SitemapStep : RecipeExecutionStep {
        ...
        private void ProcessCustomRoutes(XContainer root) {
            ...
            foreach (var customRouteDefinitionElement in customRoutesDefinitionsElement.Elements()) {
                var customRouteUrl = customRouteDefinitionElement.Attribute("Url")
                    .Value;
                ...
                try {
                    ...
                    customRouteDefinition.IndexForDisplay = bool.Parse(customRouteDefinitionElement.Attribute("IndexForDisplay")
                        .Value);
                    customRouteDefinition.IndexForXml = bool.Parse(customRouteDefinitionElement.Attribute("IndexForXml")
                        .Value);
                    customRouteDefinition.UpdateFrequency = customRouteDefinitionElement.Attribute("UpdateFrequency")
                        .Value;
                    customRouteDefinition.Priority = int.Parse(customRouteDefinitionElement.Attribute("Priority")
                        .Value);
                }
                catch (Exception ex) {
        ...
        private void ProcessRoutes(XContainer root) {
            ...
            foreach (var routeDefinitionElement in routeDefinitionsElement.Elements()) {
                var routeSlug = routeDefinitionElement.Attribute("Slug")
                    .Value;
                Logger.Information("Importing route '{0}'.", routeSlug);
                try {
                    var routeDefinition = GetOrCreateRouteDefinition(routeSlug);
                    routeDefinition.DisplayLevels = int.Parse(routeDefinitionElement.Attribute("DisplayLevels")
                        .Value);
                    routeDefinition.Active = bool.Parse(routeDefinitionElement.Attribute("Active")
                        .Value);
                    routeDefinition.DisplayColumn = int.Parse(routeDefinitionElement.Attribute("DisplayColumn")
                        .Value);
                    routeDefinition.Weight = int.Parse(routeDefinitionElement.Attribute("Weight")
                        .Value);
                }
                catch (Exception ex) {
        ...
        private void ProcessSettings(XContainer root) {
            ...
            foreach (var settingDefinitionElement in settingsDefinitionsElement.Elements()) {
                var settingContentType = settingDefinitionElement.Attribute("ContentType")
                    .Value;
                ...
                try {
                    ...
                    settingDefinition.IndexForDisplay = bool.Parse(settingDefinitionElement.Attribute("IndexForDisplay")
                        .Value);
                    settingDefinition.IndexForXml = bool.Parse(settingDefinitionElement.Attribute("IndexForXml")
                        .Value);
                    settingDefinition.UpdateFrequency = settingDefinitionElement.Attribute("UpdateFrequency")
                        .Value;
                    settingDefinition.Priority = int.Parse(settingDefinitionElement.Attribute("Priority")
                        .Value);
                }
                catch (Exception ex) {
